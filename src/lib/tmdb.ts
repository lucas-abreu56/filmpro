 ...
 * `w780` é indistinguível de `w1280` esticado — atrás do scrim no herói, e nos
 * fotogramas da tira, que no desktop não passam de ~270px de largura e no
 * celular de ~540px — e custa ~55% menos bytes. Tanto o herói (`Hero.tsx`)
 * quanto a tira (`FilmStrip.tsx`) chamam esta função com o padrão `w780`.
 * Além do peso de rede, na tira o fundo é re-rasterizado a cada quadro
 * enquanto a coluna abre no hover, e a textura menor barateia esse custo.
 *
